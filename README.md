[Partial Offer Coin (PoC)](https://github.com/dexie-space/dexie-governance/issues/12)

## create a genesis partial offer
```
❯ partial create -f $partial_fp --offer 1:1 --request 657bdae0165c622f635374e539ef7e4632750ecc87541071478c21a7ba67096c:128.680 -pk 
```

## get
```
❯ partial get -ph d00aa66868bba6e5bdda64eae25116cd42eeb8410b7e328551c7fd6b46ece933 -pk a07f3f0f388f33cb4e2b5494e7be564a7f8cbedeabc453be0c3ed75e36ddf407b8667bf9b84d7045e1c0c16462ad822d -t 657bdae0165c622f635374e539ef7e4632750ecc87541071478c21a7ba67096c -r 128.680 -a 1000000000000  -h
b0db8f7952a418110c8b0368a59ec6134fe84bc2b043a4ba9c6afc8bf834c0fd
```

## show
```
❯ partial show ./genesis.offer
{
  "maker_puzzle_hash": "d00aa66868bba6e5bdda64eae25116cd42eeb8410b7e328551c7fd6b46ece933",
  "public_key": "a07f3f0f388f33cb4e2b5494e7be564a7f8cbedeabc453be0c3ed75e36ddf407b8667bf9b84d7045e1c0c16462ad822d",
  "tail_hash": "657bdae0165c622f635374e539ef7e4632750ecc87541071478c21a7ba67096c",
  "cat_offer_mod_hash": "29d195d8255fe21dabe9a2e9dbd275fbe4cd7cb239c7e1c9c1d7e4bbbb8341f2",
  "rate": 100000,
  "offer_mojos": 250000000000,
  "is_genesis": true,
  "partial_coin": {
    "parent_coin_info": "0xc217e2fc0f438593c92f4843413275e92b36d39957a6212f235e4761b787ac70",
    "puzzle_hash": "0x90c3cc8f197848d3a16a6fdf995b6b816703542ffe6fa875cbe44b5b1afd2ed8",
    "amount": 250000000000
  }
}
```

```
❯ partial show ./next-1.offer
{
  "maker_puzzle_hash": "d00aa66868bba6e5bdda64eae25116cd42eeb8410b7e328551c7fd6b46ece933",
  "public_key": "a07f3f0f388f33cb4e2b5494e7be564a7f8cbedeabc453be0c3ed75e36ddf407b8667bf9b84d7045e1c0c16462ad822d",
  "tail_hash": "657bdae0165c622f635374e539ef7e4632750ecc87541071478c21a7ba67096c",
  "cat_offer_mod_hash": "29d195d8255fe21dabe9a2e9dbd275fbe4cd7cb239c7e1c9c1d7e4bbbb8341f2",
  "rate": 100000,
  "offer_mojos": 100000000000,
  "is_genesis": false,
  "partial_coin": {
    "parent_coin_info": "0x15fd22da39003b892c2620ebf1b9e23a3b767175230d8fd616176f28706f95db",
    "puzzle_hash": "0xcdc1048e83b877c362efd67b9f5da4daf4cb07f600ebd80b7dac8ba5ad0c057f",
    "amount": 100000000000
  }
}
```

## take
```
❯ partial take -f $partial_taker_fp -a $(0.15e12) genesis.offer
{
  "spend_bundle": {
    "coin_spends": [...],
    "aggregated_signature": "..."
  },
  "next_offer": "offer1..."
}
```