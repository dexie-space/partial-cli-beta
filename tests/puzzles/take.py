...
coin_ph = partial_puzzle.get_tree_hash()
...
class TestPartialTake:
    ...
    def test_partial_take(self):
        ...
        expected_request_mojos = req_mojos_puzzle.run([uint64(taken_amount)]).as_int()
        ...
        apa = CreatePuzzleAnnouncement(
            ...
            puzzle_hash=partial_info.get_request_settlement_hash(),
        ...
        fee = FEE_MOD.run([partial_info.fee_rate, taken_amount]).as_int()
    ...
    def test_all_take(self):
        ...
        # (ASSERT_PUZZLE_ANNOUNCEMENT  0xd2d8bc6e388f8787ab9d7996c56545ec8cb7803ea7bda04990e6d3bee92dc387)
        expected_request_mojos = req_mojos_puzzle.run([uint64(taken_amount)]).as_int()
        assert expected_request_mojos == uint64(600e3)
        maker_request_payments = Program.to(
            [
                coin_id,
                [
        ...
        # create new partial coin should not exist
        new_amount = coin_amount - taken_amount
        assert (
            condition_exists(
                conditions,
